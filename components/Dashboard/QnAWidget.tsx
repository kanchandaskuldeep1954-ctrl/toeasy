...
export const QnAWidget: React.FC<QnAWidgetProps> = ({ dataset, context }) => {
    ...
    const handleAsk = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const answer = await GroqService.consultAgent(dataset, query, dashboardContext, messages.map(m => ({ role: m.role, text: m.content })).slice(-4));
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200 dark:border-slate-700 shadow-2xl rounded-2xl w-[380px] h-[500px] flex flex-col overflow-hidden pointer-events-auto"
                    >
                        {/* Header */}
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse" />
                                <h3 className="text-xs font-black uppercase tracking-widest text-slate-600 dark:text-slate-300">Data Analyst</h3>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 font-medium">
                            {messages.length === 0 && (
                                <div className="text-center mt-10 opacity-50">
                                    ...
                                    <p className="text-xs text-slate-500 max-w-[200px] mx-auto">Ask me anything about your dashboard data, trends, or specific metrics.</p>
                                </div>
                            )}
