...
const Sidebar: React.FC = () => {
  ...
  const menuItems: MenuItem[] = [
    ...
    { id: 'dashboard', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', label: 'Dashboards', path: '/app/dashboards', alwaysEnabled: true },
    { id: 'report', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', label: 'Reports', path: '/app/reports', alwaysEnabled: true },
  ...
  return (
    <aside className="hidden lg:flex w-20 xl:w-64 flex-col bg-slate-900 border-r border-slate-800 shrink-0 transition-all duration-300">
      ...
      <div className="p-4 mt-auto border-t border-slate-800 space-y-3">
        ...
        <button
          onClick={toggleTheme}
          className="w-full flex items-center justify-center xl:justify-start gap-3 px-3 py-3 xl:px-4 xl:py-3 rounded-xl bg-slate-800/50 hover:bg-slate-800 text-slate-400 hover:text-white transition-all group"
          title={theme === 'dark' ? 'Switch to Light Mode' : 'Switch to Dark Mode'}
        >
          {theme === 'dark' ? (
            <svg className="w-6 h-6 xl:w-5 xl:h-5 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707M12 5a7 7 0 100 14 7 7 0 000-14z" /></svg>
          ) : (
            <svg className="w-6 h-6 xl:w-5 xl:h-5 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
          )}
          <span className="hidden xl:inline text-xs font-bold uppercase tracking-widest">{theme === 'dark' ? 'Light' : 'Dark'} Mode</span>
        </button>
