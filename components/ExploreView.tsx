...
const ExploreView: React.FC<ExploreViewProps> = ({ dataset, onAIAction, onUpdate }) => {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (response.toLowerCase().includes('visual') || response.toLowerCase().includes('chart') || userMsg.toLowerCase().includes('chart')) {
         ...
         if (dataset.dashboardConfig?.charts) {
             const relevant = dataset.dashboardConfig.charts.find(c => 
                 userMsg.toLowerCase().includes(c.title.toLowerCase()) || 
                 userMsg.toLowerCase().includes(c.type)
             );
