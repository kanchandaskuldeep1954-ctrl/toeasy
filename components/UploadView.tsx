...
const UploadView: React.FC<UploadViewProps> = ({ onDataLoaded }) => {
    ...
    // Robust CSV Parser (State Machine) to handle quotes, escaped quotes, and newlines within fields
    const parseCSV = (text: string) => {
        ...
        const data = rows.slice(1).map(row => {
            ...
            headers.forEach((header, i) => {
                let val = row[i] !== undefined ? row[i].trim() : '';
    ...
    const handleFile = (file: File) => {
        ...
        reader.onload = (e) => {
            ...
            try {
                ...
                else alert("The CSV file seems to be empty or misformatted.");
            } catch (err) {
    ...
    const handleConnect = async () => {
        ...
        try {
            ...
            // Using quick start data instead of generateMockData to reduce API calls
        ...
        <div className="flex flex-col items-center justify-center space-y-8 py-4">
            <div className="flex flex-col items-center text-center space-y-2">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl font-bold text-white shadow-xl" style={{ backgroundColor: connector.brandColor }}>
                    {connector.icon && typeof connector.icon === 'string' && connector.icon.includes('/') ? <img src={connector.icon} alt="" className="w-8 h-8" /> : connector.icon}
                </div>
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
                {connector.fields.map((field, idx) => {
                    ...
                    const isPassword = fieldLower.includes('password') || fieldLower.includes('secret');
                        ...
                        {filteredConnectors.map(c => (
                            ...
                            >
                                <div className="w-10 h-10 rounded-lg bg-white dark:bg-slate-700 shadow-sm flex items-center justify-center text-lg font-bold text-slate-700 dark:text-slate-200 mb-3 group-hover:scale-110 transition-transform z-10" style={{ color: c.brandColor }}>
                                    {c.icon && typeof c.icon === 'string' && c.icon.includes('/') ? <img src={c.icon} alt="" className="w-6 h-6" /> : c.icon}
                                </div>
