...
const CleanView: React.FC<CleanViewProps> = ({ dataset, onUpdate, onAIAction }) => {
  ...
  const displayHeaders = dataset.headers.filter(h => h !== '__metadata');
  ...
  const handleAutoGenerateLogic = async () => {
    if (!ruleFormData.description) return;
  ...
  const handleAddNlRule = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nlRuleInput.trim() || isGeneratingRule) return;
  ...
  return (
    <div className="h-full flex flex-col gap-6 w-full max-w-[1900px] mx-auto overflow-hidden p-4 md:p-6 lg:p-8">
      ...
      {/* Header Panel */}
      <div className="glass-panel p-4 rounded-[40px] shadow-2xl flex flex-col md:flex-row gap-4 justify-between items-center shrink-0 z-50 border border-indigo-500/10">
        <div className="flex gap-2 overflow-x-auto no-scrollbar w-full md:w-auto pb-2 md:pb-0">
            {[
              ...
              <button
                ...
                className={`px-8 py-3 rounded-[24px] text-[10px] font-black uppercase tracking-widest flex items-center gap-3 transition-all whitespace-nowrap ${
                  activeTab === tab.id 
      ...
      <div className="flex-1 min-h-0 relative">
        {activeTab === 'validation' && (
          <div className="h-full flex flex-col gap-6 relative overflow-hidden">
             {/* Action Bar */}
             <div className="glass-panel p-8 md:p-12 rounded-[56px] flex flex-col lg:flex-row justify-between items-center gap-10 shadow-xl border border-indigo-500/10 shrink-0">
                <div className="flex-1 space-y-2 text-center lg:text-left">
                    <h3 className="text-3xl font-black uppercase tracking-tighter">Truth-Gate Architect</h3>
                        ...
                        <div className="flex justify-between items-start mb-8 ml-10">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => {
                                    ...
                                    onUpdate({ ...dataset, validationRules: updated });
                 ...
                 <div className="fixed inset-0 z-[100] flex justify-center items-center p-4">
                    ...
                    <div className="w-full max-w-[650px] bg-white dark:bg-slate-900 shadow-2xl rounded-[56px] p-10 md:p-14 flex flex-col gap-8 animate-in zoom-in-95 relative overflow-hidden border-4 border-white/10">
                        <div className="flex justify-between items-center">
                            <h3 className="text-3xl font-black uppercase tracking-tighter">{editingRuleId ? 'Modify Logic Gate' : 'Forensic Gate Designer'}</h3>
                        ...
                        <div className="space-y-8 overflow-y-auto no-scrollbar pr-2 max-h-[60vh]">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {['Recovery', 'Audit'].map(cat => (
                                        <button 
                                            ...
                                            className={`px-4 py-3 rounded-2xl text-[8px] font-black uppercase tracking-widest border-2 transition-all ${
                                                ruleFormData.category === cat ? 'bg-indigo-600 text-white border-indigo-500 shadow-xl' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-500'
                                            }`}
                                        >
                            ...
                            <div className="space-y-4">
                                ...
                                <select 
                                    ...
                                    className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-[24px] text-sm font-bold outline-none"
                                >
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    ...
                                    <select value={ruleFormData.column} onChange={(e) => setRuleFormData({...ruleFormData, column: e.target.value})} className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-[24px] text-sm font-bold outline-none">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <select value={ruleFormData.relationshipType} onChange={(e) => setRuleFormData({...ruleFormData, relationshipType: e.target.value as any})} className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-[24px] text-sm font-bold outline-none">
        ...
        {activeTab === 'editor' && (
          <div className="h-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[64px] flex flex-col overflow-hidden relative shadow-2xl">
            ...
            <div className="flex-1 flex overflow-hidden min-h-0 relative">
                ...
                <div className={`w-[450px] glass-panel border-l border-slate-100 dark:border-slate-800 p-8 flex flex-col gap-8 shadow-2xl shrink-0 transition-transform duration-500`}>
