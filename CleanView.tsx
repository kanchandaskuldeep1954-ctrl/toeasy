...
const CleanView: React.FC<CleanViewProps> = ({ dataset, onUpdate, onAIAction }) => {
  ...
  const handleApplySingle = (action: CleaningAction) => executeCleaning([action]);
                                 ...
                                 <div className="flex justify-between items-start mb-2">
                                     <div className="flex items-center gap-3">
                                         ...
                                         <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                                             action.type === 'missing_values' ? 'bg-amber-100 text-amber-700' :
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
                      {insights.map((ins, i) => (
                          <div key={i} className="flex gap-3 items-start">
                              <div className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${ins.importance === 'high' ? 'bg-rose-500' : 'bg-emerald-500'}`}></div>
                              <div>
                              ...
                              <tr key={idx} className="group hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                                  ...
                                  {dataset.headers.map(h => {
                                      ...
                                      return (
                                          <td 
                                            key={h} 
                                            className={`px-4 py-2 text-xs truncate max-w-[200px] border-r border-transparent transition-colors ${
                                                changed 
