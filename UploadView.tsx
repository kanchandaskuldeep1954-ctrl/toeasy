...
import { GroqService } from './src/services/groqService'; // Keeping specific service import
...
const UploadView: React.FC<UploadViewProps> = ({ onDataLoaded }) => {
    ...
    // Robust CSV Parser (State Machine) to handle quotes and newlines
    const parseCSV = (text: string) => {
        ...
        const data = rows.slice(1).map(row => {
            ...
            headers.forEach((header, i) => {
                // Safe access if row length mismatch
    ...
    const handleFile = (file: File) => {
        ...
        reader.onload = (e) => {
            ...
            try {
                ...
                if (data.length > 0) {
                    onDataLoaded(data, file.name, 'csv');
                } else {
                    alert("Could not parse CSV. File appears empty or invalid.");
                }
            } catch (err) {
                console.error(err);
    ...
    const handleConnect = async () => {
        ...
        try {
            ...
            const mockData = await GroqService.generateMockData(selectedConnector.id);
